            ...
            <div className="transition duration-500 md:max-w-2xl md:w-full m-3 md:mx-auto lg:mt-12 animate__animated animate__fadeInDown">
                <div className={`flex flex-col rounded-2xl py-4 px-5 ${isLightMode ? "bg-white" : "bg-gray-900 border"}`}>
                    <div className="flex justify-between items-center pb-4 border-b border-gray-200">
                        <h4 className={`text-sm font-medium ${isLightMode ? "text-gray-900" : "text-white"}`}>{projectlists[ID].title}</h4>
                    ...
                    <div className="overflow-y-auto py-4 flex flex-col justify-center items-center">
                        <div className='w-full flex justify-center items-end'>
                            <div className='lg:w-1/2 md:w-1/2 sm:w-full w-full flex flex-col justify-center items-start mb-8'>
                                <p className={`font-medium lg:text-2xl md:text-2xl sm:text-lg text-lg ${isLightMode ? "text-gray-900" : "text-white"}`}>Project Title : {projectlists[ID].title}</p>
                                <p className={`font-medium lg:text-lg text-sm ${isLightMode ? "text-gray-900" : "text-white"}`}>Project Role : {projectlists[ID].role}</p>
                            </div>
                        ...
                        <div className='w-full flex justify-center items-center py-2 overflow-y-auto lg:h-auto md:h-auto sm:h-60 h-40'>
                            <p className={`font-medium ${isLightMode ? "text-gray-900" : "text-white"}`}>{projectlists[ID].summary}</p>
                        </div>
                    </div>
                    <div className="flex items-center justify-end pt-4 border-t border-gray-200 space-x-4">
                        <button type="button" className="py-2.5 px-5 text-xs bg-indigo-50 text-indigo-500 rounded-full cursor-pointer font-semibold text-center shadow-xs transition-all duration-500 hover:bg-indigo-100 close-modal-button" data-pd-overlay="#medium-modal" data-modal-target="medium-modal" onClick={() => setIsOpen(false)}>Cancel</button>
                        <a href={projectlists[ID].url} target={projectlists[ID].url} className="py-2.5 px-5 text-xs  bg-indigo-500 text-white rounded-full cursor-pointer font-semibold text-center shadow-xs transition-all duration-500 hover:bg-indigo-700 close-modal-button" data-pd-overlay="#medium-modal" data-modal-target="medium-modal">Visit Website</a>
                    </div>
                </div>
            </div>
        </div>
